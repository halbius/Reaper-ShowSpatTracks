...
recTracksArray = {}
sendTracksArray = {}
allTrackCount = reaper.CountTracks(0)
numSelTrack = reaper.CountSelectedTracks(0)

 if numSelTrack ~= nil then
 arrayCount = 0
 

  for i = 1, numSelTrack do -- cycle through all selected tracks
  
    selectedTrack = reaper.GetSelectedTrack(0, i-1) -- check if track is selected and store in <seltrack>
    sendsCount = reaper.GetTrackNumSends(selectedTrack, 0) -- get number of sends of track
    
        for i = 0, sendsCount -1  do
        
 --        track = reaper.GetTrack(0 ,i)
        
          _, sendName = reaper.GetTrackSendName(selectedTrack, i)
        
          if string.find(sendName, "ToSpat") then 
        
            sendTracksArray[arrayCount] = selTrack
            recTracksArray[arrayCount] = reaper.GetTrackSendInfo_Value(selectedTrack, 0, 0, 'P_DESTTRACK')
            arrayCount = arrayCount + 1
            reaper.ShowConsoleMsg(tostring(recTracksArray[arrayCount]) .. "\n")
...
if recTracksArray[i] == 0 then
 ...
 -- reaper.ShowConsoleMsg(tostring(recTracksArray[i]) .. "\n")
end
end
